...
def all_combiner_evaluation(data_batch, selected_quest_compare, selected_quest_compose, selected_quest,
                            ...
                            retrieve_number=100,exp_result=None,output=None,result_path=None):
    ...
    for data in data_batch:
        ...
        if str(data["id"]) not in output:
            ...
            #scouts = gene_exps[:6]
            scouts = gene_exps
            # get the number of every item in gene_exps
            counts =  [gene_exps.count(item) for item in gene_exps]
            if max(counts) == 1:
                scouts = [gene_exps[0]]
            else:
            # 取出现次数最多的一个，若都只出现一次则保留全部
                scouts = [max(scouts, key=scouts.count)]
            logger.info("scouts: {}".format(scouts))
            # if gene_exps.count(scouts[0]) == 1:
            #     scouts = gene_exps
